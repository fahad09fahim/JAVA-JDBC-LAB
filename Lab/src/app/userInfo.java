...
public class userInfo extends javax.swing.JFrame {
    ...
    private void deleteActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FIRST:event_deleteActionPerformed
        // Check if the ID field is empty first
    ...
    if (response == JOptionPane.YES_OPTION) {
        ...
        try {
            ...
            // 3. SECURE QUERY (Using PreparedStatement)
            // This prevents SQL Injection and handles the deletion safely
    ...
    try {
        ...
        // STEP 1: Get the CURRENT data first
        ...
        if (rs.next()) {
            // We found the user, let's get their current info
            String oldName = rs.getString("name");
            String oldEmail = rs.getString("email");
            String oldPhone = rs.getString("phone"); // This will be the valid INT from DB
            
            // STEP 2: Decide which data to use (New vs Old)
            
            // For Name
            String finalName;
            if ("".equals(uName.getText())) {
                finalName = oldName; // Field is empty, keep old name
            } else {
                finalName = uName.getText(); // Field has text, use new name
            }
            ...
            // For Phone (This fixes your specific crash!)
            String finalPhone;
            if ("".equals(uPhone.getText())) {
                finalPhone = oldPhone; // Keeps the valid number from DB
            } else {
                finalPhone = uPhone.getText(); // Uses the new input
            }
            
            // STEP 3: Perform the Update with the "Final" values
